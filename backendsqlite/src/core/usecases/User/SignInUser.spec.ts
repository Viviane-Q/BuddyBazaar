...
const whenUserSignsIn = (email: string, password: string): Promise<string | null> => {
    return SignInUser({ email, password, userRepository, encryption });
};

const thenTokenShouldBe = (expectedToken: string | null, receivedToken: string | null) => {
...
describe('Feature: Sign in an user', () => {
    test('Example: Jean wants to sign in as the first user registered', async () => {
        ...
        const receivedToken = await whenUserSignsIn(UserFixtures.userJeanWithHash.email, '123456');
        thenTokenShouldBe('token', receivedToken);
    });
    test('Example: Martin wants to sign in but is not registered', async () => {
        givenUsers([UserFixtures.userJeanWithHash]);
        const receivedToken = await whenUserSignsIn(UserFixtures.userMartin.email, '123456');
